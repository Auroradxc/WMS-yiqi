<template>
  <div >
    ...
    >
      ...
      <el-table-column prop="customerId" label="编号" width="600"></el-table-column>
...
import{get_customer_list} from '../../request/request.js'
...
export default {
  ...
  methods: {
    async fetchCustomer(){
      let data =  await get_customer_list();
      console.log(data);
    ...
    search() {
        ...
        if (this.filters.id) {
          filtered = filtered.filter(item => item.customerId.includes(this.filters.id));
        }
  
        // 根据供应商筛选
        if (this.filters.name) {
          filtered = filtered.filter(item => item.customerName.includes(this.filters.name));
        }
    ...
    async handleDelete(row){
      ...
      await Http.post("http://97.64.20.181:8000/delete_customer",{"customerId":row.customerId}).then((res) => {
        console.log(res)
      })
  },
  handleAdd(){
