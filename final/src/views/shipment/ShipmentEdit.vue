...
import{exstorelist_detail} from '../../request/request.js';
export default {
  components: {
    AddSup
  },
  ...
  created() {
    this.form.orderNumber = this.$route.query.DeliveryNum;
  ...
  methods: {
    async fetchEnstore(){
      let info = {"listNum":this.form.orderNumber};
      let data = await exstorelist_detail(info);
      console.log(data);
      data = data.wrapInfoDetails.map(item=>({
        partNumber:item.supId,
        quantity:item.expInNum,
        boxCount:item.boxes,
        toolCode:item.wrapId,
        packageSize:item.wrapSize
      }));
      console.log("data",data);
      this.form.parts = data;
    },
    ...
    async save() {
      ...
      console.log(message);
      try {
        const response = await axios.post('http://97.64.20.181:8000/modify_exstore_list/update_supplement', 
          message)// 传递入库单号作为参数
          // 如果请求需要携带数据，请在这里添加);
        if (response.data ) {
