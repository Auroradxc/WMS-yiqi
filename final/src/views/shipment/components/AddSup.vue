<template>
    <el-dialog v-model="dialogVisible" title="添加物料" width="60%">
      <div class="inputArea">
  ...
  import { get_stock_list } from '../../../request/request.js';
  ...
  export default {
    ...
    computed: {
      filteredData() {
        ...
        if (this.query) {
          return partsArray.filter(part => part.supId && part.supId.includes(this.query.supId)&&part.available=='Available'&&part.supplier && part.supplier.includes(this.query.supplier));
        }
        return partsArray.filter(part => part.available === 'Available').slice((this.currentPage - 1) * this.pageSize, this.currentPage * this.pageSize);
      },
    },
    watch: {
      parts(newValue) {
    ...
    methods: {
      ...
      async get_supplement_list_filtered() {
        let info = {
          supId: this.query || null,
          supplier: this.supplier || null,
        };
        try {
          const response = await get_stock_list(info);
      ...
      async fetchStockList() {
        try {
          const response = await axios.post('http://97.64.20.181:8000/request_stock', {
            "supId": null,
            "supplier": null
          });
